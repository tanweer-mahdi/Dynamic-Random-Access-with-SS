...
N = 128;
M = 64;
Mlamb = -26; % threshold for varying p
J = 9;
p = 0.1;
tp = 20;
radius = 0.2;
...
SER_SPICE = [];
    ...
    rel_spice = 0;
for num=1:200000
    aer_esp = zeros(length(JJ),1);
    nnmse_esp = zeros(length(JJ),1);
    nser_esp = zeros(length(JJ),1);
    aer_spice = zeros(length(JJ),1);
    nnmse_spice = zeros(length(JJ),1);
    nser_spice = zeros(length(JJ),1);
    ...
    for mm = 1:length(JJ)
        ...
        %% Generating data
        params = mtc_data(N,M,J,p,tp,radius,st,JJ(mm),constellation);
...
%        auset_esp = auset_ref;
        ...
        tstart = tic;
        auset_spice = spice_aud(params.snaps,phi,Mlamb);
